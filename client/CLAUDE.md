...
> ## 🔧 Son Değişiklikler (2026-08-22, 21. tur) — Onaylanmış Belgeler Artık Silinebiliyor (Deactivate)
> Kullanıcı, sil butonunun onaylanmış bir belgede gri/tıklanamaz durduğunu fark edip bunun mantıklı olup olmadığını sordu. Netleştirme sonrası: "silme" artık geri alınabilir bir deactivate olduğu için (dosya storage'da kalıyor, admin görebiliyor/geri aktifleştirebiliyor - bkz. 19. tur), onaylanmış bir belgeyi geri dönüşsüz kaybetme riski yok; kullanıcı bu kısıtlamanın kaldırılmasını istedi.
> - **`UserDocumentsCard.tsx`**: `deleteBlockedReason` hesaplamasından `doc.status === 'approved'` dalı çıkarıldı - artık sadece `is_primary` kontrolü kalıyor (o kısıtlamaya dokunulmadı, kullanıcı sadece onaylı belgeler hakkında soru sordu). Backend tarafı: `backend/claude.md` 19. tur.
> - **Doğrulama**: `npx tsc -b` + `npx vite build` temiz. Backend tarafı (`status==APPROVED` engelinin kaldırılması, `is_primary` engelinin korunduğu) gerçek bir DB kopyasında `APIRequestFactory` ile doğrulandı - onaylı+birincil-olmayan bir belge artık `204` ile siliniyor, `status` alanı `approved` olarak korunuyor (geçmiş kaybolmuyor). **Bu turdaki client değişikliği gerçek bir tarayıcıda tıklanarak henüz test edilmedi.**
...
---
**Son Güncelleme**: 2026-08-22, 22. tur (Kullanıcının "PDF görüntülenemeyebilir" endişesi üzerine, `UserDocumentsCard.tsx`'teki zorla-indirme davranışı [`handleDownload`, 20. turda CORS hatası düzeltilen fonksiyonun ta kendisi] tamamen kaldırıldı, yerine expert'teki "Görüntüle" [yeni sekmede aç] deseni getirildi - iki panel yükleme sırasında zaten sadece PNG/JPG/PDF kabul ediyor, üçü de tarayıcıda inline açılıyor, ayrı bir indirme mekanizmasına gerek yoktu. Bu değişiklik CORS/blob-fetch karmaşıklığının kendisini tamamen ortadan kaldırıyor. Ayrıca kullanıcının "uzman panelinde belge silinince kutucuk kaybolmuyor" sorusu koda bakılarak yanıtlandı - `refreshProfile()` zaten backend'in `is_current=True` filtresiyle güncel listeyi çektiği için bunun zaten çalışması gerekiyor, kod değişikliği gerekmedi. `tsc -b`/`vite build` temiz, gerçek tarayıcıda henüz test edilmedi)
