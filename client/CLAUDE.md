...
---
**Son Güncelleme**: 2026-08-26, 23. tur (Backend'e eklenen `payments/` app'inin [bkz. backend/claude.md 22-23. tur] client tarafı: yeni sidebar sayfası "Ödemeler" [`pages/Payments/Payments.tsx`] bekleyen ödemeleri (fiyat + "Öde" butonu + onay modalı) ve ödeme geçmişini listeliyor, `POST /payments/appointments/{id}/checkout/` yanıtına göre mock-anında-başarı ile gerçek-mod-redirect'i ayırt ediyor. Yeni `pages/Payments/PaymentResult.tsx` gerçek modun döneceği sonuç sayfası. `NotificationDropdown.tsx`'e `payment_required` için yeni bir yönlendirme dalı eklendi. `types/appointment.ts`/`types/notification.types.ts` güncellendi. `tsc -b`/`vite build` temiz, gerçek tarayıcıda henüz test edilmedi)
