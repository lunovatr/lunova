...
> ## 🔧 Son Değişiklikler (2026-08-22, 18. tur) — Belge Silme (= Aktif/Pasif) Özelliği Eklendi
> Kullanıcı, danışan panelinin yanı sıra uzman panelinde de belge silme özelliği istedi (gerçek DELETE değil, aktif/pasif). `features/profile/api.ts::deleteDocument()` fonksiyonu ÖNCEDEN VARDI ama hiçbir UI'ya bağlı değildi (ölü kod) - bu turda gerçekten kullanılır hale getirildi. Backend detayı (`DocumentDeleteView`'daki gereksiz `storage.delete()` çağrısının kaldırılması, admin panelinde aktif/pasif gösterimi) için `backend/claude.md`'nin 18. tur girdisine bakın.
> - **`profile-view.tsx`**: "Belgeler" kartındaki her satıra bir `Trash2` (lucide-react) ikon butonu eklendi, var olan `components/confirm-dialog.tsx`/`ConfirmDialog` bileşeniyle (proje genelinde zaten kullanılan onay dialog'u deseni) bir onay adımı var. Onaylanmış/birincil belgelerde buton baştan `disabled` + açıklayıcı `title` tooltip'i (backend'in reddedeceği bir isteği önceden engelliyor).
> - **`fetchProfile` → `refreshProfile` olarak ayrıştırıldı**: silme sonrası listeyi tazelemek için kullanılan yol, sayfanın `loading` state'ine bilinçli olarak DOKUNMUYOR - aksi halde client'ta 17. turda düzeltilen "tam ekran spinör flaşı" hatasının aynısı (bkz. client/claude.md) burada da oluşurdu. İlk mount'taki yükleme hâlâ `loading`'i kullanıyor (tam sayfa "Yükleniyor..." durumu sadece ilk yüklemede gösteriliyor).
> - **🔍 Bu turda `features/profile/api.ts::handleApiError`'da bulunan bir bug düzeltildi**: `DocumentDeleteView`'ın iş kuralı ihlallerinde fırlattığı `ValidationError`, DRF tarafından `{"detail": "..."}` DEĞİL ham bir dizi (`["mesaj"]`) olarak serialize ediliyor - `handleApiError`'ın var olan genel "obje" dalı (diziler de JS'te `typeof === 'object'`dir) bunu yanlışlıkla `Object.entries()`'e verip `"0: mesaj"` gibi bir index önekiyle gösteriyordu. `Array.isArray()` kontrolü en başa eklenerek düzeltildi - bu düzeltme `deleteDocument()` dışında `handleApiError` kullanan HER fonksiyonu (upload, profile update) da aynı sınıf bir hatadan koruyor. `deleteDocument()`'a ayrıca `uploadDocument()` ile tutarlı bir başarı toast'ı eklendi (önceden yoktu).
> - **Doğrulama**: `npx tsc -b` + `npx vite build` temiz (dist temizlendi). Backend tarafı (deactivate mantığı, hata response şekli) gerçek bir DB kopyasında `APIRequestFactory` ile doğrulandı (bkz. backend/claude.md). **Bu turdaki expert değişiklikleri gerçek bir tarayıcıda tıklanarak henüz test edilmedi.**
...
---
**Son Güncelleme**: 2026-08-26, 20. tur (Backend'e eklenen `payments/` app'inin [bkz. backend/claude.md 22-23. tur] uzman tarafı: `features/reservations/api.ts::Appointment`'a `payment_status`/`session_price`/`session_currency` eklendi, `appointments-table.tsx` + `appointment-detail-dialog.tsx`'e var olan status-badge deseni taklit edilerek bir ödeme durumu rozeti eklendi. `features/notifications/api.ts`'teki `NotificationType`'a `payment_succeeded` eklendi - navigasyon mantığına dokunulmadı, zaten var olan genel `appointment_id` fallback'i yeterliydi. `tsc -b`/`vite build` temiz, gerçek tarayıcıda henüz test edilmedi)
