...
---
**Son Güncelleme**: 2026-08-28, 23. tur (Backend'de 27. turda kurulan grup seansı "müsaitlik→talep→onay→ödeme" durum makinesinin [bkz. backend/claude.md] uzman tarafı: yeni `features/groups/` [oluşturma formu - `catalog/session-offerings` + `accounts/session-types` uçlarından besleniyor, grup listesi tablosu, detay dialog'u - Onayla/Reddet + onaylanmış katılımcı listesi + ex-user rozeti] + `/groups` sidebar linki eklendi. `appointments-table.tsx`/`appointment-detail-dialog.tsx`'e seans tipi rozeti + "(net kazancınız: X TRY)" eklendi. `tsc -b`+`vite build` temiz [yeni route nedeniyle önce `vite build` ile `routeTree.gen.ts` yeniden üretildi], backend tarafı gerçek dev DB'ye karşı 60 kontrolle doğrulandı, gerçek tarayıcıda henüz test edilmedi)
