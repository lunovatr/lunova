...
---
**Son Güncelleme**: 2026-08-22, 19. tur (Kullanıcının client tarafında sorduğu "onaylanmış belge neden silinemiyor" sorusu netleştirildi - silme geri alınabilir bir deactivate olduğu için eski kısıtlamanın gerekçesi kalmamıştı, `profile-view.tsx`'teki `deleteBlockedReason`'dan `status==='approved'` dalı çıkarıldı, `is_primary` kontrolü korundu. Backend değişikliği [`backend/claude.md` 19. tur] gerçek bir DB kopyasında doğrulandı. `tsc -b`/`vite build` temiz, gerçek tarayıcıda henüz test edilmedi)
