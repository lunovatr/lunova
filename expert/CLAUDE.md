...
---
**Son Güncelleme**: 2026-08-27, 21. tur (Ödeme rozeti artık danışanın ücretsiz ilk seans hakkını kullandığı bir seansı gerçek ödemeden ayırt ediyor. `features/reservations/api.ts::Appointment`'a `is_free_trial` eklendi, `appointments-table.tsx` + `appointment-detail-dialog.tsx`'teki sabit `PAYMENT_STATUS_LABELS` yerine `is_free_trial`'a bakan bir `paymentBadgeLabel()` yardımcı fonksiyonu geldi [renk değişmedi, sadece metin: "Ücretsiz İlk Seans"/"Ücretsiz Seans Onayı Bekleniyor"]. Detail dialog'daki fiyat parantezi artık `is_free_trial` iken gösterilmiyor. `tsc -b`/`vite build` temiz, backend tarafı gerçek dev DB'ye karşı 28/28 kontrolle doğrulandı [bkz. backend/claude.md 25. tur], gerçek tarayıcıda henüz test edilmedi)
