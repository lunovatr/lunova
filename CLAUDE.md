...
> ## 🔧 Son Değişiklikler (2026-09-01, 35. tur) — 🟠 Aynı Tarayıcıda Uzman+Danışan Paneli Token Çakışması Düzeltildi
>
> Kullanıcı, aynı tarayıcıda hem uzman hem danışan panelinde giriş yaptığında oturumların birbirini ezdiğini bildirip bunun CLAUDE.md'de not edilip edilmediğini sordu. İki paralel Explore agent ile önce dokümantasyon (kök + `backend/claude.md` + git geçmişi, sıfır eşleşme) sonra kod (cookie mekanizması) tarandı, ardından bir Plan agent'la implementasyon planı tasarlanıp plan-mode'da onaylandı.
>
> - **🟠 [DÜZELTİLDİ] Kök neden**: `backend/accounts/views/views.py::set_auth_cookies()` her iki frontend için de sabit `access_token`/`refresh_token` cookie adlarını kullanıyordu. Dev'de cookie'ye `domain` verilmediği için host-only (`localhost`) oluyor - RFC 6265 gereği port bazlı izole değil (`5173`/`5174` aynı jar'ı paylaşıyordu). Prod'da `domain='lunova.tr'` ile `uzman.lunova.tr`/`danisan.lunova.tr` aynı registrable domain'in subdomain'leri olarak yine paylaşıyordu.
> - **Çözüm**: yeni `backend/accounts/auth_cookies.py` modülü - cookie'ler artık `{frontend_type}_access_token`/`{frontend_type}_refresh_token` (`expert_*`/`client_*`) olarak ayrışıyor. `CookieJWTAuthentication.authenticate()`, `LoginView`, `TokenRefreshView`, `LogoutView` güncellendi; `MeView`'a dokunulmadı (cookie'yi hiç doğrudan okumuyor). Ayrıca `resolve_frontend_type()`'ın Referer-fallback listesindeki bir substring bug'ı (`uzman.lunova.tr` prod domain'i eksikti) da aynı çıkarma sırasında düzeltildi. Fallback YOK - geriye dönük uyumluluk bilinçli olarak "deploy sonrası bir kez daha login" ile çözüldü (access 15dk/refresh 1 saat ömürlü olduğu için etki süresi doğal sınırlı).
> - **Doğrulama**: gerçek dev `db.sqlite3`'e karşı ephemeral bir script ile 30/30 kontrol + gerçek dev sunucusuna karşı gerçek bir `curl` cookie-jar zinciriyle (iki panelin TEK bir birleşik jar'da birbirini EZMEDİĞİ, expert logout'un client oturumunu HİÇ etkilemediği) uçtan uca doğrulandı - httpOnly cookie mimarisi ve CSRF mekanizması korunarak. `manage.py check` temiz (model/migration yok, 3 dosya). Detay: backend/claude.md 30. tur.
> - **Bilinçli olarak bu turda YAPILMADI**: frontend'lere dokunulmadı (zaten `X-Frontend-Type`'ı her istekte gönderiyorlardı); gerçek tarayıcıda iki-sekme manuel testi bu ortamda hâlâ yapılamadı (tarayıcı otomasyonu yok, projenin genel kısıtı).

> ## 📜 34. tur — arşivlendi (özet)
>
> Grup seansı iptalinde açıkta kalan danışanların admin panelinden manuel başka bir gruba aktarılabilmesi (GEÇİCİ çözüm, kullanıcının kendi tabiriyle) + admin panelinin genel dokümantasyon/güvenlik güncellemesi (Tier 1/Tier 2 `admin_note()` banner'ları, yeni üst-seviye `GroupSessionParticipantAdmin`, zoom-link-gating bug düzeltmesi). Net sonuç yukarıdaki "📊 Sistem Durumu Özeti"nde duruyor, tam ayrıntı `git log -p -- CLAUDE.md` ile geri getirilebilir.
...
4. Cookie'ler set edilir (35. tur, YENİ: adlar frontend'e göre ayrışıyor):
   - {frontend_type}_access_token  → httpOnly, Secure=True, SameSite=Lax, max_age=15 dakika
   - {frontend_type}_refresh_token → httpOnly, Secure=True, SameSite=Lax, max_age=1 saat
   (frontend_type = 'expert'|'client'; çözülemezse legacy `access_token`/`refresh_token`'a düşer)
```

### 🍪 [YENİ, 2026-09-01, 35. tur] Aynı tarayıcıda uzman+danışan paneli token çakışması düzeltildi

Aynı tarayıcıda hem uzman hem danışan paneline giriş yapınca oturumlar birbirini eziyordu — kök neden cookie adlarının (`access_token`/`refresh_token`) her iki frontend için sabit olmasıydı (dev'de host-only `localhost` cookie'si port bazlı izole değil, prod'da `domain='lunova.tr'` subdomain'ler arası paylaşılıyordu). Yeni `backend/accounts/auth_cookies.py` modülü cookie adlarını `{frontend_type}_access_token`/`{frontend_type}_refresh_token` olarak ayrıştırdı — `CookieJWTAuthentication`, `LoginView`, `TokenRefreshView`, `LogoutView` güncellendi. Gerçek dev sunucusuna karşı `curl` cookie-jar zinciriyle uçtan uca doğrulandı: iki panel TEK bir birleşik jar'da (gerçek "aynı tarayıcı" senaryosu) birbirini ezmiyor, biri logout olunca diğeri etkilenmiyor. Detay: backend/claude.md "🍪 [YENİ, 30. tur]" alt başlığı.
...
- Cookie'ler **her ortamda** (dev dahil) `Secure=True` ve `SameSite=Lax` ile set ediliyor (2026-08-17'de `None`'dan değiştirildi - tüm frontend'ler backend ile aynı "site" olduğu için, bkz. CSRF bölümü). Adlar 35. turdan itibaren frontend'e göre ayrışıyor (`expert_*`/`client_*`) - bkz. yukarıdaki "🍪" alt başlığı.
