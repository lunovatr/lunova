...
**Son Güncelleme**: 2026-08-24, 27. tur (`accounts/tests/feed_accounts.py::seed_named_team_accounts()`'un ürettiği isimlendirilmiş ekip hesaplarından 6'sının [selin, selen, onur, ece, eslem, yusuf] UZMAN tarafı gerçek ad/soyad + `@lunova.tr` mailiyle güncellendi, yeni bir ekip üyesi "Samet Gültekin" eklendi - kullanıcı randevu maillerini [26. tur] test ederken ekip arkadaşlarına gerçekten ulaşabilmek istedi. `appointments/tests/feed_appointments.py` + `messaging/tests/feed_messaging.py`'deki AYNI listenin kopyaları da senkronize edildi [aksi halde bu script'ler temiz bir DB'de yeniden çalıştırıldığında bu isimler için sessizce atlanırdı]. Canlı `db.sqlite3`'te 6 kullanıcı yerinde güncellendi [PK/ilişkiler korunarak], "samet" çifti idempotent `seed_named_team_accounts()` ile sıfırdan oluşturuldu - orphan/duplike kayıt olmadığı doğrulandı. `manage.py check` temiz. Detay: backend/claude.md 22. tur)
