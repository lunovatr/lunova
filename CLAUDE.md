...
12. **[32. tur'da ertelenen, Faz 9'un kalanı] UX ilaveleri.** Canlı indirim kodu önizlemesi (kod girilince fiyatın anında güncellenmesi, ayrı bir `POST /payments/discount-codes/preview/` gerektirir), bekleme listesi sıra göstergesi ("3. sıradasınız", `GroupSessionWaitlist.joined_waitlist_at` sıralamasından hesaplanabilir), tutarlı seans-tipi renk kodlaması (Bireysel/Grup/Psikoeğitim için iki frontend'de aynı renk paleti), uzman "Kazançlarım" özet widget'ı (tamamlanmış ödemelerden toplam `expert_earning`) - hiçbiri zaman kısıtı nedeniyle bu turda yapılmadı, plan dokümanının kendi "opsiyonel" listesindeydi.
...
**Son Güncelleme**: 2026-08-28, 32. tur ("Seans Tipi Kataloğu & Fiyatlandırma Motoru"nun 31. turda kurulan backend mimarisinin frontend yapılandırması: grup seansı akışı kullanıcının netleştirmesiyle "anında öde+katıl"dan "müsaitlik→talep→onay→ödeme" durum makinesine [`GroupSessionParticipant.status`] çevrildi, `appointments/group_views.py` [YENİ] 5 uç ekledi. `catalog`/`accounts` app'lerine ilk kez gerçek REST uçları [`session-offerings/`, `session-types/`] açıldı, `payments`'a paket uçları eklendi, kıdem bazlı fiyatlandırma [31. turda yazılmış ama hiç çağrılmayan] gerçek ödeme akışına bağlandı [sıfır davranış değişikliği garantili]. Danışan tarafında: `Payments.tsx` genelleştirildi [bireysel+grup ödemeleri TEK listede, indirim kodu inputu], yeni "Grup Seansları" [Keşfet+Grup Seanslarım - grup arkadaşları dahil] ve "Paketler" sayfaları. Uzman tarafında: yeni `features/groups/` paneli [oluşturma formu, Onayla/Reddet, katılımcı listesi], randevu tablosu/dialog'una seans tipi rozeti + net kazanç gösterimi. 79 kontrol [60 yeni + 19 regresyon] gerçek dev DB'ye karşı `APIRequestFactory` ile doğrulandı, `manage.py check`+`makemigrations --check --dry-run` temiz, iki frontend `tsc -b`+`vite build` temiz. Son adım olarak [kullanıcının kendi sırasıyla] `feed_catalog.py`/yeni `feed_payments.py`/yeni `feed_group_sessions.py` `feed_db.py`'ye eklendi. **Hiçbir yeni ekran gerçek tarayıcıda test edilmedi** - bkz. "🟠 En öncelikli açık madde" madde 15. Detay: backend/claude.md 27. tur)
