...
> ## 🔧 Son Değişiklikler (2026-08-24, 27. tur) — İsimlendirilmiş Ekip Hesapları Gerçek @lunova.tr Mailleriyle Güncellendi
>
> Kullanıcı, 26. turdaki randevu mailleri özelliğini test ederken ekip arkadaşlarının test hesaplarına gerçekten ulaşabilmek istedi - `accounts/tests/feed_accounts.py::seed_named_team_accounts()`'un ürettiği 9 isimlendirilmiş ekip hesabından 6'sının (selin, selen, onur, ece, eslem, yusuf) UZMAN tarafını gerçek ad/soyad + `@lunova.tr` mailiyle değiştirmemizi, ayrıca ekipte olup listede önceden hiç yer almayan "Samet Gültekin"i de yeni bir çift olarak eklememizi istedi.
>
> - **`accounts/tests/feed_accounts.py`**: `NAMED_TEAM_MEMBERS`'a `"samet"` eklendi (artık 10 kişi), `NAMED_TEAM_SURNAMES`'a bir isim daha eklendi (danışan-twin'lerin otomatik soyadı çakışmasın). Yeni `NAMED_TEAM_REAL_INFO` sözlüğü (isim → gerçek ad/soyad/mail) eklenip `seed_named_team_accounts()`'a bağlandı - **SADECE UZMAN tarafını etkiler**; eşleşmiş DANIŞAN hesabı (`danisan_<isim>@mail.com`) bilinçli olarak eski otomatik-üretilen sahte isimle kaldı (gerçek bir danışan kimliği değil, sadece test amaçlı eşleşmiş çift - gerçek kişinin adını orada tekrarlamak kafa karıştırırdı).
> - **Cross-file tutarlılık**: `appointments/tests/feed_appointments.py` ve `messaging/tests/feed_messaging.py`, `NAMED_TEAM_MEMBERS`'ı (diğer feed script'leriyle aynı "bağımsız çalıştırılabilir" desende) AYRICA kopyalıyor ve uzman kullanıcıyı `f"{isim}@mail.com"` şeklinde SABİT olarak arıyordu - bu iki dosya güncellenmeseydi, gelecekte bu script'ler yeniden çalıştırıldığında (örn. temiz bir DB'de) `yusuf`/`eslem`/vb. için "ekip hesapları bulunamadı" diyerek randevu/mesaj senaryolarını SESSİZCE atlardı. İkisine de aynı `NAMED_TEAM_REAL_EMAILS` kopyası + `_expert_email_for()` yardımcı fonksiyonu eklendi, ilgili `User.objects.filter(email=...)` çağrıları buna yönlendirildi.
> - **Canlı `db.sqlite3` da güncellendi** (sadece script'i değiştirmek yetmezdi - dev DB'de bu 6 hesap zaten seed edilmişti): 6 mevcut kullanıcının `email`/`first_name`/`last_name` alanları doğrudan `save(update_fields=...)` ile yerinde güncellendi (PK değişmedi, tüm var olan ilişkiler - randevular, mesajlar, `ClientProfile.expert` eşleşmesi - korundu). Ardından güncellenen `seed_named_team_accounts()` canlı DB'ye karşı çalıştırılıp "samet" çifti sıfırdan oluşturuldu; diğer 9 kişi idempotency sayesinde "zaten mevcut" diyerek atlandı (duplikasyon yok).
> - **Doğrulama**: `manage.py check` temiz, 3 dosyanın hepsi `ast.parse` ile syntax kontrolünden geçti. Canlı DB'de: (1) 10 ismin TAMAMI için uzman email/ad-soyad + danışan email + `ClientProfile.expert_id == ExpertProfile.id` eşleşmesi doğru olduğu sorgulanıp gözle teyit edildi; (2) eski 6 adresin (`selin@mail.com` vb.) DB'de hiç kalmadığı (`User.objects.filter(email__in=[...]).count() == 0`) doğrulandı - orphan/duplike kayıt yok; (3) toplam kullanıcı sayısının beklenen şekilde (+2, samet çifti) arttığı doğrulandı.
> - **Not**: kullanıcının mesajındaki "Lunova İletişim <merhaba@lunova.tr>" satırı bir "Kimden" başlığı gibi göründüğü için, bunun mailer'ın gönderen kimliğini (`Lunova Destek <noreply@lunova.tr>`, 25. tur) değiştirme talebi OLMADIĞI, sadece kullanıcının kopyaladığı referans listesinin bir parçası olduğu varsayıldı - mailer'da hiçbir değişiklik yapılmadı. Yanlışsa kolayca düzeltilebilir.
>
> **[Aynı gün, küçük ek düzeltme]** Kullanıcı ekibin yarısını gerçekten UZMAN, yarısını gerçekten DANIŞAN olarak test etmek istediğini belirtti: yusuf/eslem/onur uzman tarafında, samet/selen/selin/ece ise **danışan** tarafında gerçek `@lunova.tr` kimliğini taşıyacak (`danisan_` öneki YOK, gerçek mailleri doğrudan danışan hesabında). `NAMED_TEAM_REAL_INFO`'ya her isim için bir `"role"` (`expert`/`client`) eklendi, `seed_named_team_accounts()` artık HER iki tarafı da (uzman/danışan) role'e göre gerçek/sahte bilgi arasında seçiyor (önceden sadece uzman tarafı gerçek olabiliyordu). `appointments`/`messaging` feed'lerindeki kopyalar da (`NAMED_TEAM_ROLES` + yeni `_client_email_for()`) senkronize edildi. Canlı `db.sqlite3`'te samet/selen/selin/ece'nin gerçek kimliği uzman hesabından danışan hesabına TAŞINDI (iki User satırı arasında email/ad/soyad swap edildi, `ClientProfile.expert` eşleşmesi bozulmadı) - `manage.py check` temiz, tüm 7 isim + eski sahte adreslerin artık doğru role'de olduğu + hiçbir orphan/duplike kayıt olmadığı sorgulanıp doğrulandı. Kullanıcı bir sonraki aşamada bu rolleri TERS ÇEVİRECEĞİNİ belirtti - `role` alanı tam bunun için var, o değişiklik geldiğinde sadece `NAMED_TEAM_REAL_INFO`/`NAMED_TEAM_ROLES`'teki `role` değerleri + canlı DB'de aynı swap tekrarlanacak.
...
10. **ücretsiz randevu hakkı** danışanın 15 dakikalık ücretsiz seans hakkı daha net planlanmalı. danışan randevu oluşturacağı zaman 15 dakikalık ücretsiz seans hakkı çeşitli noktalarda renkli gösterim ile ifade edilecek. alternatif olarak 15 dakikalık ücretsiz seans planlamasını indirim kodu kısmında da tanımlayabiliriz, en iyi yaklaşım agent ile tartışılacak, çünkü gereksiz bir free seans alanına gerek yok.
...
**Son Güncelleme**: 2026-08-26, 29. tur (28. turda kurulan iyzico ödeme akışı gerçek randevu yaşam döngüsüne + her iki frontend'e bağlandı. Backend: uzman onayladığında/randevu oluşturduğunda ödeme gerekiyorsa `send_appointment_confirmed_email` yerine yeni `send_payment_required_email` + yeni `payment_required` bildirimi danışana gidiyor; gerçek bir ödeme tamamlandığında [mock/DIRECT-callback/PREAUTH-postAuth'un üçünden de] hem danışana hem uzmana `payment_succeeded` bildirimi. `AppointmentSerializer`'a `payment_status`/`session_price`/`session_currency` eklendi - ayrı bir endpoint gerekmeden hem client'ın yeni Ödemeler sayfasını hem expert'in ödeme rozetini besliyor. Client: yeni sidebar sayfası "Ödemeler" [bekleyen ödemeler + ödeme geçmişi, onay modalı, mock/gerçek mod ayrımı, `/payments/result` sonuç sayfası], bildirim tıklaması `payment_required` için buraya deep-link veriyor. Expert: randevu tablosu + detay dialog'una var olan status-badge deseni taklit edilerek "Ödeme" rozeti eklendi. Gerçek dev DB'ye karşı GERÇEK view'lar üzerinden [`force_authenticate`] 16 yeni kontrol geçti [mail dallanması, bildirim üretimi, iki randevu oluşturma akışının ikisi de], tüm test verisi temizlendi. `manage.py check` + `makemigrations --check` temiz, iki frontend'de de `tsc -b`+`vite build` temiz. **Hiçbiri gerçek tarayıcıda test edilmedi, gerçek iyzico sandbox key'i de yok** - bkz. "🟠 En öncelikli açık madde" madde 12. Detay: backend/claude.md 23. tur, client/claude.md 23. tur, expert/claude.md 20. tur)
