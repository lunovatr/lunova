...
> ## 🔧 Son Değişiklikler (2026-08-22, 24. tur) — Belge İndirme Kaldırıldı, Yerine Expert'teki "Görüntüle" Deseni Getirildi
> Kullanıcı iki soru sordu: (1) uzman panelinde belge silindiğinde kutucuğun kaybolup kaybolmadığı, (2) belge görüntülemenin uzman tarafındaki gibi (yeni sekmede aç) mı yoksa client'taki gibi (zorla indir) mi olması gerektiği - PDF gibi formatların görüntülenemeyebileceği endişesiyle.
> - **Madde 1 için kod değişikliği gerekmedi**: `profile-view.tsx` incelendi, `refreshProfile()` zaten backend'in `is_current=True` filtresiyle güncel listeyi çektiği için (18. turda doğrulanmıştı) bunun zaten çalışması gerekiyordu.
> - **Madde 2**: iki tarafta da yüklemenin sadece PNG/JPG/PDF kabul ettiği doğrulandı (`client/UploadDocumentModal.tsx`, `expert/profile-form.tsx`) - üçü de tarayıcıda doğrudan açılınca inline render olur, "PDF görüntülenemez" riski bu proje için geçerli değil. Kullanıcı iki panelin de "Görüntüle" desenine geçmesini onayladı: `client/src/components/UserProfile/UserDocumentsCard.tsx`'teki `handleDownload()` (20. turda CORS hatası düzeltilen fonksiyonun ta kendisi - blob fetch + zorla indirme) TAMAMEN kaldırıldı, yerine expert'teki `<a href target="_blank">` deseniyle birebir aynı bir "Görüntüle" linki geldi. Bu, CORS/blob-fetch karmaşıklığının kendisini bir daha hiç gerekmeyecek şekilde ortadan kaldırıyor.
> - **Doğrulama**: `npx tsc -b` + `npx vite build` temiz. Expert tarafında kod değişikliği yapılmadı (zaten aynı deseni kullanıyordu), backend'de de değişiklik yok. **Bu turdaki değişiklik gerçek bir tarayıcıda henüz test edilmedi.**

> ## 🔧 Son Değişiklikler (2026-08-22, 23. tur) — Onaylanmış Belgeler Artık Silinebiliyor (Deactivate)
> Kullanıcı, 21. turdaki belge silme özelliğini denerken sil butonunun onaylanmış bir belgede gri/tıklanamaz durduğunu fark etti, bunu "saçma" bulup görüşümü sordu: onaylanmış belge silinebilmeli mi, silinmemeli mi? Netleştirme: kullanıcı silinebilmesini istedi.
> - **Gerekçe**: kısıtlama, "silme"nin gerçek/geri dönüşsüz bir DELETE olduğu döneme (18. tur öncesi) aitti - artık geri alınabilir bir deactivate olduğu için (dosya storage'da kalıyor, admin panelinden görünüp yeniden aktifleştirilebiliyor, `status` alanına hiç dokunulmuyor) o gerekçe geçersizdi.
> - **Backend**: `accounts/views/document_views.py::DocumentDeleteView.perform_destroy()`'daki `status==DocumentStatus.APPROVED` engeli kaldırıldı, `is_primary` engeli AYNEN korundu (kullanıcı sadece onaylı belgeler hakkında soru sordu).
> - **İki frontend**: `UserDocumentsCard.tsx` ve `profile-view.tsx`'teki `deleteBlockedReason` hesaplamasından `status==='approved'` dalı çıkarıldı.
> - **Doğrulama**: gerçek bir DB kopyasında `APIRequestFactory` ile - önce onaylı hale getirilen bir belge artık `204` ile siliniyor (önceden `400`), `status` alanı deaktivasyon SONRASI da `approved` olarak korunuyor (geçmiş kaybolmuyor), `is_primary=True` bir belge hâlâ `400` alıyor (regresyon yok). `manage.py check` temiz, iki frontend'de `tsc -b`/`vite build` temiz. Detay: backend/claude.md 19. tur, client/claude.md 21. tur, expert/claude.md 19. tur.

> ## 🔧 Son Değişiklikler (2026-08-22, 22. tur) — 🐛 Belge İndirme Kırıktı: CORS + Credentialed Cross-Origin İstek Çakışması
> Kullanıcı, 21. turdaki belge silme özelliğinin ilk gerçek tarayıcı testi sırasında, danışan panelinde bir belgeyi indirmenin `Access-Control-Allow-Origin: '*' ile kimlik bilgisi desteklenmez` CORS hatasıyla başarısız olduğunu bildirdi - bu turların ilk defa gerçek bir tarayıcıda bulunan gerçek bug'ı.
> - **Kök neden**: `client/src/components/UserProfile/UserDocumentsCard.tsx::handleDownload()`, `doc.access_url`'i (backend'in kendi API'si DEĞİL, Supabase'in ürettiği tam bir dış imzalı URL) paylaşılan `api` axios instance'ı (`withCredentials:true` + özel header'lar) ile çekiyordu. Bu, tarayıcının isteği "credentialed" bir cross-origin istek saymasına yol açıyordu; Supabase Storage ise CORS'ta `Access-Control-Allow-Origin: '*'` dönüyor - Fetch/CORS spesifikasyonu gereği wildcard ACAO + credentials kombinasyonu HER ZAMAN reddedilir, tarayıcı isteği engelliyordu.
> - **Düzeltme**: `api.get()` yerine credential'sız düz bir `fetch(url)` kullanıldı (fetch'in varsayılanı zaten cross-origin'de cookie göndermiyor, token zaten URL'de) + önceden sessizce başarısız olan hata durumuna görünür bir toast eklendi. `UserMetaCard.tsx`'teki `<img>` gösterimi ve expert'teki "Görüntüle" `<a href target='_blank'>` linki aynı sorunu YAŞAMIYOR (ikisi de plain browser navigasyonu/img yüklemesi, `api` instance'ını hiç kullanmıyor) - sorun sadece bu bir blob-indirme deseniyle sınırlıydı, sistematik bir tarama gerekmedi.
> - **Doğrulama**: `npx tsc -b` + `npx vite build` temiz. **Gerçek bir tarayıcıda gerçek bir Supabase belgesiyle henüz yeniden test edilmedi** - kullanıcının bir sonraki denemesi bekleniyor. Detay: `client/claude.md` 20. tur.

> ## 📜 Daha Eski Turlar (2026-08-22, 21. tur ve öncesi) — arşivlendi
> Kural (e) gereği ("📌 Kalıcı Kural" → Şişmeyi önleme) 21. tur ve öncesinin ayrıntılı prose'u bu dosyadan çıkarıldı — net sonuçları zaten yukarıdaki **"📊 Sistem Durumu Özeti → ✅ Kapatılmış kritik/yüksek öncelikli maddeler"** listesinde tek satırlık özetler olarak duruyor (belge silme = aktif/pasif [deactivate] özelliği danışan+uzman panelinde eklendi [21. tur], belge onay/red akışı [admin] + admin panel genel güçlendirmesi + form versiyon numarası görünürlüğü eklendi [20. tur], 🔴 kritik danışan belge/profil fotoğrafı yükleme akışı düzeltmesi + deploy/altyapı teşhis serüveni [Render/Supabase Frankfurt taşınması, IPv6/Session pooler, `.env` bozuk değerleri] + `default-avatar.png` sonsuz döngü bug'ı + kullanıcının 7 maddelik UX listesi + testte bulunan 3 ek bug [19. tur], veritabanı besleme script'lerine isimlendirilmiş ekip hesapları + `messaging`/`notifications` feed'leri + `feed_db.py` orkestrasyon script'i eklendi [18. tur], Notlar sistemi saatlik throttle'dan seans-bazlı mesaj kotasına çevrildi [17. tur], "Notlar" uzman-danışan not/mesaj sistemi eklendi [16. tur], danışan formları matrisi + 🔴 kritik `client_id` çakışma bug'ı düzeltmesi [15. tur], global bildirim sistemi eklendi [14. tur], Formlar'daki `yes_no` seçenek kaybı bug'ının gerçek kök nedeni bulunup düzeltildi [13. tur], Formlar sekmesinin ilk tarayıcı testinden 4 bulgu [12. tur], danışan formları + otomatik versiyonlama + kritik skorlama hatası düzeltmesi [11. tur], Zoom mock URL placeholder'a çevrildi [10. tur], doküman sistemi iyileştirmesi + `expert/ToDo.md` konsolidasyonu [9. tur], ana sayfa scroll bug'ı + expert çift-modal bug'ı + Zoom 15dk kısıtı + takvim renklendirmesi [8. tur], mobil header/sidebar bug'ları + marka/UI temizliği + client ana sayfası + Clerk sökümü [7. tur], toast z-index bug'ı [6. tur], CSRF koruması [5. tur], access token refresh [3. tur], profil "beyaz sayfa" zinciri [devam turu], randevu 3-ID karışıklığı + login/me `id`/`role` eksikliği [Randevu Zinciri turu], `AvailabilityExceptionView`/`ExpertAvailability` navigate no-op [4. tur]). Tam ayrıntı (kod örnekleri, curl doğrulama adımları) kayıp değil — `git log -p -- CLAUDE.md` ile bu dosyanın o zamanki hâli her zaman geri getirilebilir (repo artık `git subtree` ile birleşmiş tek bir depo, bkz. aşağıdaki "⚠️ Repo Yapısı" notu).
>
...
Notification (notifications/models.py) — 14. tur, YENİ; 16. turda genişletildi; 20. turda 3. türe çıktı
├── user (FK), notification_type (appointment_reminder | message | document_status), dedupe_key, title, body
...
**Son Güncelleme**: 2026-08-22, 24. tur (Kullanıcının "PDF görüntülenemeyebilir" endişesi üzerine, danışan panelindeki zorla-indirme davranışı [`UserDocumentsCard.tsx::handleDownload()` - 20. turda CORS hatası düzeltilen fonksiyonun ta kendisi] tamamen kaldırıldı, yerine uzman panelindeki "Görüntüle" [yeni sekmede aç] deseni getirildi - iki panel yükleme sırasında zaten sadece PNG/JPG/PDF kabul ediyor, üçü de tarayıcıda inline render olur, ayrı bir indirme mekanizmasına gerek yoktu; bu CORS/blob-fetch karmaşıklığının kendisini tamamen ortadan kaldırıyor. Kullanıcının "uzman panelinde belge silinince kutucuk kaybolmuyor" sorusu da koda bakılarak yanıtlandı - `refreshProfile()` zaten backend'in `is_current=True` filtresiyle güncel listeyi çektiği için bunun zaten çalışması gerekiyordu, kod değişikliği gerekmedi. Backend/expert'te değişiklik yok, client'ta `tsc -b`/`vite build` temiz, gerçek tarayıcıda henüz test edilmedi. Detay: client/claude.md 22. tur)
