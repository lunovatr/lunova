...
> ## 🔧 Son Değişiklikler (2026-08-20, 16. tur) — Yeni Özellik: Uzman-Danışan Not/Mesaj Sistemi ("Notlar")
> Kullanıcı, eşleşen her uzman-danışan çifti için klasik canlı chat DEĞİL, kompakt bir "not bırakma" sistemi istedi: "yazıyor" gibi canlı bir mekanizma yok, mesajlar sayfa yenileme/polling ile gelir. Amaç, randevu oluştururken girilen seansa özgü `notes` alanından AYRI olarak, seans öncesi/sonrası bilgilendirme ve genel iletişimin görüntülü görüşmeyle sınırlı kalmaması. Kullanıcı, karakter limiti ve gönderim sıklığı limiti gibi "best practice" limitlerin bilinçli olarak uygulanmasını istedi (ileride seans başına kota sistemi eklenecek — bu turda kota YOK, sadece genel limit). Kod tabanında bu işleve dair hiçbir kalıntı (gerçek ya da şablon) yoktu — üç katmanda da sıfırdan inşa edildi. Detay: `backend/claude.md`/`client/claude.md`/`expert/claude.md`'nin 14./15./14. tur girdileri.
> - **Backend — yeni `messaging/` app**: `Conversation` (expert/client User FK, `UniqueConstraint(expert,client)`, sadece ilk mesaj gönderiminde `get_or_create` ile oluşuyor — GET'te değil) + `Message` (`body`=`CharField(max_length=1000)`, `is_read`/`read_at`). `GET/POST /api/v1/messaging/conversations/<other_user_id>/messages/` — `other_user_id` HER ZAMAN `User.id` (15. turda `forms/views.py`'de bulunan `ClientProfile.id`/`User.id` çakışma bug'ının tekrarlanmaması için bilinçli tasarım). Gönderim sıklığı için projede İLK KEZ DRF `ScopedRateThrottle` kullanıldı (`'messaging_send': '30/hour'`, SADECE bu endpoint'in POST'una scoped — global throttle YOK, başka hiçbir endpoint etkilenmiyor).
> - **Dormant gap düzeltmesi**: `accounts/serializers/profileSerializers.py` → `ClientProfileSerializer.get_expert()` önceden sadece `full_name`/`title` dönüyordu, `id` HİÇ yoktu — ama client'ın `ProfileResponse.expert` tipi zaten `id: number` bekliyordu (hiç okunmadığı için gerçek bir hata hiç üretmemiş "uyuyan" bir eksiklik). Artık `"id": obj.expert.user_id` dönüyor.
> - **Bildirim zili entegrasyonu (kullanıcı onayıyla)**: `notifications/models.py`'ye `'message'` türü + nullable `related_user` FK eklendi; her yeni not gönderiminde alıcı için idempotent bir `Notification` oluşuyor (`create_message_notification()`). Client'ta bildirime tıklayınca `/messages`'a (tek karşı taraf olduğu için ID gerekmez), expert'te `/messages?clientId=...`'e yönlendiriyor.
> - **Client (`pages/Messages/Messages.tsx`)**: client'ın her zaman TEK karşı tarafı (kendi atanmış uzmanı) olduğu için roster yok, doğrudan mesaj akışı + karakter sayaçlı `<textarea>` + 60sn polling. **Expert (`features/messages/`)**: sol tarafta danışan roster tablosu (son not önizlemesi + okunmamış rozeti), sağda seçilen danışanla olan not paneli — `client-forms`'un aksine detay bir Dialog değil, satır içi panel.
> - **Doğrulama — tamamı gerçek DB verisiyle, `APIRequestFactory` ile gerçekten çalıştırılarak**: permission kontrolleri (her iki yönde), read-receipt işaretleme, 1000 karakter/boş body validasyonu, 30/hour throttle'ın TAM 31. istekte `429` verdiği, bildirim oluşturma + idempotency, roster endpoint'i — hepsi gerçek bir expert/client çiftiyle doğrulandı. `manage.py check` temiz, migration'lar sorunsuz uygulandı. İki frontend'de de `tsc`/`vite build` temiz. **Hiçbir frontend değişikliği gerçek bir tarayıcıda tıklanarak test edilmedi** (bu ortamda hâlâ tarayıcı otomasyon aracı yok — projenin her turunda tekrarlanan aynı sınırlama).

> ## 📜 Daha Eski Turlar (2026-08-20, 15. tur ve öncesi) — arşivlendi
> Kural (e) gereği ("📌 Kalıcı Kural" → Şişmeyi önleme) 15. tur ve öncesinin ayrıntılı prose'u bu dosyadan çıkarıldı — net sonuçları zaten yukarıdaki **"📊 Sistem Durumu Özeti → ✅ Kapatılmış kritik/yüksek öncelikli maddeler"** listesinde tek satırlık özetler olarak duruyor (danışan formları matrisi + 🔴 kritik `client_id` çakışma bug'ı düzeltmesi [15. tur], global bildirim sistemi eklendi [14. tur], Formlar'daki `yes_no` seçenek kaybı bug'ının gerçek kök nedeni bulunup düzeltildi [13. tur], Formlar sekmesinin ilk tarayıcı testinden 4 bulgu [12. tur], danışan formları + otomatik versiyonlama + kritik skorlama hatası düzeltmesi [11. tur], Zoom mock URL placeholder'a çevrildi [10. tur], doküman sistemi iyileştirmesi + `expert/ToDo.md` konsolidasyonu [9. tur], ana sayfa scroll bug'ı + expert çift-modal bug'ı + Zoom 15dk kısıtı + takvim renklendirmesi [8. tur], mobil header/sidebar bug'ları + marka/UI temizliği + client ana sayfası + Clerk sökümü [7. tur], toast z-index bug'ı [6. tur], CSRF koruması [5. tur], access token refresh [3. tur], profil "beyaz sayfa" zinciri [devam turu], randevu 3-ID karışıklığı + login/me `id`/`role` eksikliği [Randevu Zinciri turu], `AvailabilityExceptionView`/`ExpertAvailability` navigate no-op [4. tur]). Tam ayrıntı (kod örnekleri, curl doğrulama adımları) kayıp değil — `git log -p -- CLAUDE.md` ile bu dosyanın o zamanki hâli her zaman geri getirilebilir (repo artık `git subtree` ile birleşmiş tek bir depo, bkz. aşağıdaki "⚠️ Repo Yapısı" notu).
>
...
**Son Güncelleme**: 2026-08-21, 18. tur (Veritabanı besleme script'leri güncellendi: `accounts`/`appointments` feed'lerine isimlendirilmiş ekip test hesapları [selin, selen, onur, ece, eslem, gokcen, niga, mustafa, yusuf — her biri `<isim>@mail.com` uzman + `danisan_<isim>@mail.com` eşleşmiş danışan, ortak domain/şifre] eklendi; `messaging`/`notifications` app'leri için ilk kez feed script'i yazıldı (örnek Türkçe "Notlar" sohbetleri + mesaj kotasının farklı durumlarını gösteren senaryolar + randevu hatırlatma bildirimi senkronizasyonu); backend kökünde tüm feed'leri doğru sırayla çalıştıran `backend/feed_db.py` orkestrasyon script'i oluşturuldu. Gerçek `db.sqlite3`'ün bir kopyası üzerinde (gerçek veriye dokunulmadan) uçtan uca çalıştırılarak doğrulandı — `get_client_remaining_quota()` her ekip çifti için tasarlanan değeri birebir verdi, bu doğrulama sırasında kendi bulunan bir sınır-durumu tasarım hatası [mustafa'nın 3-günlük bildirim senaryosu] fark edilip düzeltildi. Detay: backend/claude.md 16. tur)
