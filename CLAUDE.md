...
6. **[25. tur'da kullanıcı tarafından planlandı] Randevu talebi/onay/iptal durumu maili.** Randevu durum değişikliklerinde ilgili taraflara (`appointments/views.py`'deki status-update akışı) mail gönderilmesi planlanıyor. Kullanıcı bu tetikleme mantığını bir sonraki aşamada kendisi kuracağını belirtti — `mailer` app'i bu iş için hazır (`send_email()` + gerekli `send_<tür>_email()` sarmalayıcılarının eklenmesi yeterli), henüz appointments'a bağlanmadı.
...
**Son Güncelleme**: 2026-08-24, 25. tur (Backend'de mail gönderimi merkezi bir `mailer/` app'ine taşındı - önceden `PasswordResetRequestView` içine gömülü tek kullanımlık `send_mail` çağrısı artık `mailer.services.send_email()`/`send_password_reset_email()` üzerinden gidiyor; `zoom/` app'iyle aynı şekilde model/migration'sız, sadece servis katmanı. Karar, kullanıcının ileride ekleyeceği çok sayıda mail türünü [seans hatırlatması, admin duyuru/kampanya, randevu durumu, danışan mesajı bildirimi] tek bir merkezi noktadan geçirme isteğiyle alındı. Gerçek bir DB kopyasında `APIRequestFactory` ile 4 senaryo doğrulandı: dev-mode log, production'da gerçek `send_mail` çağrısı, SMTP hatası simülasyonunda exception/`fail_silently` davranışı, view'ın 500 response'u. Detay: backend/claude.md 20. tur)
