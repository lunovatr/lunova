...
> ## 🔧 Son Değişiklikler (2026-08-22, 19. tur) — 🔴 Kritik: Danışan Belge/Fotoğraf Yükleme Tamamen Bozuktu + Deploy/Altyapı Teşhis Serüveni + Kapsamlı UX Düzeltme Turu
> Bu tur, kullanıcının prod'a (Render + Supabase + Netlify) deploy sürecini birlikte yürütürken art arda ortaya çıkan gerçek sorunları kapsıyor — deploy/DNS/Supabase teşhisi, ardından danışan panelinde profil fotoğrafı yüklemeyi "sadece kontrol et" isteğiyle başlayıp 🔴 kritik bir bug'a çıkan zincir, sonra kullanıcının verdiği 7 maddelik UX listesi, onu test ederken bulunan 3 ek bug. Client tarafının tam detayı için `client/claude.md`'nin 17. tur, expert için 16. tur girdisine bakın.
> - **Deploy/altyapı (kod değil, ortam/config)**: Render backend servisi Frankfurt'a, Supabase projesi de (Türkiye'ye coğrafi olarak en yakın makul seçenek olduğu için) Frankfurt'a taşındı — önceki Render Oregon + Supabase Seoul kombinasyonu, her DB sorgusunun yarı dünyayı turlamasına sebep oluyordu, taşındıktan sonra kullanıcı gözle gözlemlenebilir bir hızlanma bildirdi. Ayrı bir Render deploy'unda `httpx.ConnectError: Name or service not known` hatası alındı — teşhis DNS/Docker sorunu gibi görünse de (bkz. aşağıdaki "kendi bulduğum yanlış teşhis" notu) gerçek sebep Supabase'in `db.<proje>.supabase.co` "direct connection" host'unun artık SADECE IPv6 olması, Render'ın bu instance'ının IPv6 desteklememesiydi — çözüm Supabase'in IPv4-uyumlu **Session pooler** connection string'ine geçmekti (`DB_URI`'yi güncelledi).
> - **🔴 KRİTİK — `client/src/components/UserProfile/UploadDocumentModal.tsx`, var olmayan bir endpoint'e (`/api/v1/accounts/documents/upload/`) istek atıyordu** — gerçek akış `presign-upload/`→PUT→`documents/` (finalize) şeklinde 3 adımlı, expert'in `features/profile/api.ts::uploadDocument()`'ı bunu zaten doğru uyguluyordu. Sonuç: danışan tarafından yapılan HER belge/fotoğraf yükleme denemesi 404 ile başarısız oluyordu. Expert'in akışıyla birebir aynı hale getirilerek düzeltildi. Bu düzeltmeyi test ederken (lokal Docker + gerçek Supabase'e karşı) sırasıyla iki AYRI, ilgisiz sorun daha çıktı: (1) `backend/.env`'deki `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` değerleri **gerçekten bozuktu** — kullanıcı muhtemelen bir yerden (panel gösterimi vb.) kısmen maskelenmiş görünen metni doğrudan kopyalayıp yapıştırmış, dosyada literal `*` karakterleri kalmıştı (`ccm****swxae` gibi) — bu, DNS çözülemez bir host adı üretiyordu. **Kendi bulduğum yanlış teşhis**: ilk başta bunu Docker Desktop'ın DNS sorunu sandım, `docker-compose.yml`'e gereksiz yere DNS sunucusu (`8.8.8.8`/`1.1.1.1`) ekledim — gerçek sorun DNS altyapısı değil, `.env`'deki değerin kendisiydi; bunu, değerin İÇERİĞİNİ hiç görmeden (uzunluk/`*` içeriyor mu/DNS çözülüyor mu gibi güvenli, sadece boolean sonuç veren bir Python kontrolüyle) doğrulayarak buldum. (2) Doğru URL'e geçilince bu sefer `403 new row violates row-level security policy` alındı — kullanıcı panelden `service_role`/`secret` (RLS bypass eden) yerine `publishable`/`anon` (RLS'e tabi) anahtarı kopyalamıştı, önek kontrolüyle (`sb_secret_` vs `sb_publishable_`) teşhis edildi. Kullanıcı ikisini de düzeltince akış gerçek Supabase'e karşı uçtan uca (presign→PUT→finalize, 200/201) çalıştı.
> - **🟠 YÜKSEK — `default-avatar.png` dosyası hiç mevcut değildi, `UserMetaCard.tsx`'teki `onError` handler'ı bunu sonsuz bir hata döngüsüne çeviriyordu** — fotoğrafı olmayan (yukarıdaki bug yüzünden pratikte NEREDEYSE TÜM danışanlar) her kullanıcı profil sayfasına girdiğinde, fallback görsel de 404 alıp `onError`'ı yeniden tetikliyor, bu sonsuza kadar sürüyordu (kullanıcının "sürekli ard arda istek" şikayetinin gerçek sebebi — ilk elde `/me/`/`/profile/` API'lerini şüpheli görüp kod taramasıyla ekarte ettim, asıl suçlu tarayıcının bir statik dosya isteğiydi). Gerçek bir `default-avatar.svg` eklendi + `onerror=null` guard'ı iki yerde de (`UserMetaCard.tsx`, `UserDropdown.tsx`) eklendi.
> - **Netlify SPA yönlendirme eksikliği (expert'te)**: `expert/public/_redirects` hiç yoktu (`client`'ta vardı) — F5/derin link'te Netlify'ın kendi 404'ü geliyordu. Eklendi, build çıktısında doğrulandı.
> - **Kullanıcının 7 maddelik UX listesi + testte bulunan 3 ek bug**: tam ayrıntı için `client/claude.md`'nin 17. tur girdisine bakın — özetle: avatar konumu, login/kaydet/randevu-gönder butonlarına spinner, header dropdown sadeleştirme+Türkçeleştirme, belge yükleme modalındaki `<select>`'in focus'ta okunamaz hale gelmesi (`focus:bg-slate-950`'nin `dark:` ile sınırlı olmaması), profil fotoğrafı DIŞINDAKİ yüklemelerde gereksiz tam-ekran-spinner flaşı (`fetchMe()`'nin `RequireAuth`'u tetiklemesi), randevu müsaitlik panellerinin accordion yerine otomatik açık gelmesi, uzmanlar arası slot seçim state'inin ortaklaştırılması (bir uzmanın slotu seçilince diğerininki temizlenmiyordu), randevu gönderiminde başarı sonrası 2 saniyelik pencerede butonun tekrar tıklanabilir kalması (çift-tıklama/çift-randevu riski), ve önceden var olan bozuk bir spinner SVG path'i.
> - **Doğrulama**: belge yükleme akışı gerçek Docker + gerçek Supabase'e karşı UÇTAN UCA çalıştırılarak doğrulandı (en güçlü doğrulama seviyesi bu turda). Geri kalan tüm client değişiklikleri her adımda `npx tsc -b`/`npx vite build` ile doğrulandı. **Hiçbir UX düzeltmesi gerçek bir tarayıcıda tıklanarak test edilmedi** (ortamda hâlâ tarayıcı otomasyon aracı yok) — commit/push/deploy sonrası kullanıcının kendi tarayıcısında doğrulaması bekleniyor.

> ## 🔧 Son Değişiklikler (2026-08-21, 18. tur) — Veritabanı Besleme: İsimlendirilmiş Ekip Hesapları + messaging/notifications Feed'leri + Ana Orkestrasyon Script'i
> Kullanıcı, ekip üyelerinin (selin, selen, onur, ece, eslem, gokcen, niga, mustafa, yusuf) kendi isimleriyle giriş yapıp platformu test edebilmesi için besleme script'lerinin güncellenmesini istedi; ayrıca `messaging`/`notifications` app'lerinin (14-17. turlarda eklendi) hiç feed script'i yoktu, ve backend kökünde tüm feed'leri sırayla çalıştıran tek bir orkestrasyon script'i istendi. Detay: `backend/claude.md`'nin 16. tur girdisi.
> - **`accounts/tests/feed_accounts.py`**: her isim için `<isim>@mail.com` (uzman, `approval_status=True`) + `danisan_<isim>@mail.com` (o uzmana ATANMIŞ danışan) çifti — aynı domain (`mail.com`) ve şifre (`password123`) diğer test hesaplarıyla ortak. `seed_client_profiles()`'tan ÖNCE çalıştığı için named uzmanlar rastgele danışan atamasına da (mevcut ~70% ihtimal mantığıyla) dahil oluyor.
> - **`appointments/tests/feed_appointments.py`**: `NAMED_PAIR_APPOINTMENT_SCENARIOS` — her ekip çifti için bilinçli olarak farklı bir randevu geçmişi (hiç seans yok / tek tamamlanmış seans / çok seanslı geçmiş / yakın zamanda onaylı randevu / onay bekleyen ilk talep) — amaç mesaj kotasının ve "3 gün içinde yaklaşan randevu" bildiriminin gerçek veriyle gösterilebilmesi. `notes` alanına `[Ekip Senaryosu]` etiketi konularak script idempotent hale getirildi (yeniden çalıştırılınca aynı senaryo tekrar eklenmiyor).
> - **Yeni `messaging/tests/feed_messaging.py`** (önceden hiç yoktu): ekip çiftleri için Türkçe örnek "Notlar" sohbetleri — her çift, `get_client_remaining_quota()`'nun FARKLI bir sonucunu göstersin diye tasarlandı (taze/hiç-seans-yok, kısmen kullanılmış, tükenmiş [kırmızı border testi], hiç-konuşulmamış boş durum, çok-seanslı geçmiş, uzman-ağırlıklı kullanım). Kota penceresini (`messaging/services.py::_quota_window_start` ile AYNI mantık, bilinçli olarak burada tekrar yazıldı — private fonksiyona bağımlı olmamak için) gerçek `Appointment` verisinden okuyup mesaj zaman damgalarını buna göre yerleştiriyor; ayrıca genel rastgele-eşleşmiş danışan havuzundan 15'lik bir alt küme için de sade örnek sohbetler ekliyor. Her mesaj için gerçek POST akışıyla (`messaging/views.py`) aynı `create_message_notification()` çağrılıyor.
> - **Yeni `notifications/tests/feed_notifications.py`** (önceden hiç yoktu): `notifications` app'inin kendi "kaynak" verisi yok (tamamen `Appointment`/`Message`'dan senkronize ediliyor) — bu script tüm kullanıcılar için `sync_appointment_reminders()`'ı tetikleyip normalde ilk `GET /notifications/` çağrısında oluşacak hatırlatmaları önceden üretiyor, ayrıca bazı bildirimleri "yakın zamanda okunmuş" / "20 günden eski okunmuş" (bir sonraki GET'te otomatik silinecek) olarak işaretleyip rozet sayacı + otomatik temizlik davranışının UI'da gözlemlenebilmesini sağlıyor.
> - **`zoom/` için bilinçli olarak feed EKLENMEDİ**: kendi kalıcı modeli yok, `appointments` feed'i zaten `zoom_start_url`/`zoom_join_url`/`zoom_meeting_id` alanlarını randevu oluştururken dolduruyor.
> - **Yeni `backend/feed_db.py`**: backend kökünde tüm feed'leri doğru bağımlılık sırasıyla (`accounts → availability → appointments → messaging → notifications → forms`) tek Django kurulumuyla çalıştıran orkestrasyon script'i (`--apps a,b`, `--skip-forms`, `--list` argümanları var). Her alt script'in `main()`/kendi giriş fonksiyonu `importlib` ile dosya yolundan yükleniyor (paket/`__init__.py` tutarsızlıklarından bağımsız).
> - **Doğrulama — gerçek `db.sqlite3`'ün BİR KOPYASI üzerinde (env değişkeniyle `DB_URI` geçici olarak kopyaya yönlendirilip), gerçek veriye HİÇ dokunulmadan, uçtan uca gerçekten çalıştırılarak**: 9 ekip çiftinin hepsi doğru `approval_status=True`/eşleştirmeyle oluştu (idempotency ikinci çalıştırmada da doğrulandı); `get_client_remaining_quota()` her çift için TASARLANAN değeri birebir verdi (selin 3/5, selen 0/5, onur 4/5, ece 5/5-konuşma yok, eslem 2/5, gokcen 3/5, niga 0/5, mustafa 2/5, yusuf 5/5); bir örnek konuşmanın (`selen`) tüm zaman damgaları/karakter sayıları elle gözden geçirildi. Bu doğrulama sırasında **kendi bulduğum bir tasarım hatası** ortaya çıktı ve düzeltildi: `mustafa` senaryosundaki "3 gün sonraya onaylı randevu" bildirim testi, `notifications/services.py`'nin `now <= appt_dt <= now+3gün` pencere kontrolünde script'in çalıştırıldığı saate bağlı olarak sınırın dışına düşebiliyordu — 2 güne çekilerek güvenilir hale getirildi. Tüm 6 app `python feed_db.py` ile tam bir kopya üzerinde baştan sona hatasız çalıştı; gerçek `db.sqlite3` üzerinde `manage.py check` + `makemigrations --check --dry-run` temiz.

> ## 🔧 Son Değişiklikler (2026-08-20, 17. tur) — Notlar: Saatlik Throttle Yerine Seans-Bazlı Mesaj Kotası + Dinamik UI
> Kullanıcı, 16. turda eklenen "Notlar" sisteminin saatte-30-mesaj kuralını, iki seans arası toplam 5 mesaj hakkı (her seans TAMAMLANDIĞINDA yenilenen) şeklinde bir kotaya çevirmemizi istedi — danışan tarafında dinamik kalan-hak göstergesi + tıklanınca açıklama modalı açan bir info badge, hak bitince kırmızı border + disable, 200 karakter limiti (uzman sınırsız kalıyor), uzman tarafında danışanın kalan hakkının parantez içinde gösterilmesi, ve her iki frontend'de gönderilmemiş taslakların localStorage'da korunması. Detay: `backend/claude.md`/`client/claude.md`/`expert/claude.md`'nin 15./16./15. tur girdileri.
> - **Backend — throttle tamamen kaldırıldı, yerine hesaplanan kota**: `messaging/services.py::get_client_remaining_quota()` DB'de ayrı bir alan/model TUTMADAN, danışanın bu uzmanla en son `status='completed'` randevusunun bitiş zamanından (yoksa konuşmanın başından) itibaren gönderdiği mesaj sayısını sayıp `5 - used`'ı hesaplıyor — "her seans sonrası yenilenir" davranışı sadece var olan `Appointment`/`Message` verisinden türetiliyor. GET/POST yanıt şekli `{"messages"/[mesaj], "client_quota": {"remaining", "limit"}}` taşıyacak şekilde değişti (**breaking**, iki frontend güncellendi). POST'ta artık iki sunucu-taraflı kontrol var (frontend'i bypass eden bir istek için de geçerli): danışan için 200 karakter limiti (aşarsa `400 message_too_long`) ve kota kontrolü (biterse mesaj hiç oluşturulmadan `403 quota_exceeded`) — uzmanın hiçbir sınırı yok.
> - **Client**: "Notlar" sayfasına "Kalan Hak: X/5" + tıklanınca yenileme mantığını anlatan bir modal açan info-icon eklendi; hak tükenince mesaj kutusunun dış border'ı kırmızıya dönüyor, `<textarea>`/gönder butonu disable oluyor (eski mesajlar scroll ile görülmeye devam ediyor); karakter limiti 200'e indi; backend'in `quota_exceeded`/`message_too_long` yanıtları ayrı toast'larla gösteriliyor; taslak `localStorage`'da korunuyor.
> - **Expert**: roster'daki her danışan adının yanına `(kalan/limit)` parantezi + seçili danışanın panel başlığında aynı bilgi tam cümle olarak ekleniyor (uzmanın kendi gönderiminde hiçbir kısıtlama yok, sadece bilgilendirme); danışan-bazlı `localStorage` taslak koruması eklendi.
> - **Doğrulama — 10 senaryo, `APIRequestFactory` ile gerçek verilerle, gerçekten çalıştırılarak**: taze konuşmada 5/5; 5 mesajlık ardışık düşüş; 6. mesajın `403 quota_exceeded` alması; uzman mesajının danışan kotasını etkilememesi; 201 karakterlik danışan mesajının `400 message_too_long` alması (uzmanın 500 karakter gönderebilmesi); gerçek bir `completed` randevu oluşturulunca kotanın 5/5'e sıfırlanması (randevu ÖNCESİ mesajların yeni pencereye sayılmadığı); sıfırlama sonrası yeni bir mesajın kotayı doğru tüketmesi (4/5); roster'ın doğru `client_quota` döndürmesi. Test verisi, kullanıcının UI'da GERÇEKTEN mesajlaştığı çiftten (`expert10`/`client2`) bilinçli olarak farklı, dokunulmadı. `manage.py check` temiz, migration gerekmedi. İki frontend'de `tsc`/`vite build` temiz. **Hiçbir frontend değişikliği gerçek bir tarayıcıda tıklanarak test edilmedi** (bu ortamda hâlâ tarayıcı otomasyon aracı yok).

> ## 📜 Daha Eski Turlar (2026-08-20, 16. tur ve öncesi) — arşivlendi
> Kural (e) gereği ("📌 Kalıcı Kural" → Şişmeyi önleme) 16. tur ve öncesinin ayrıntılı prose'u bu dosyadan çıkarıldı — net sonuçları zaten yukarıdaki **"📊 Sistem Durumu Özeti → ✅ Kapatılmış kritik/yüksek öncelikli maddeler"** listesinde tek satırlık özetler olarak duruyor ("Notlar" uzman-danışan not/mesaj sistemi eklendi [16. tur], danışan formları matrisi + 🔴 kritik `client_id` çakışma bug'ı düzeltmesi [15. tur], global bildirim sistemi eklendi [14. tur], Formlar'daki `yes_no` seçenek kaybı bug'ının gerçek kök nedeni bulunup düzeltildi [13. tur], Formlar sekmesinin ilk tarayıcı testinden 4 bulgu [12. tur], danışan formları + otomatik versiyonlama + kritik skorlama hatası düzeltmesi [11. tur], Zoom mock URL placeholder'a çevrildi [10. tur], doküman sistemi iyileştirmesi + `expert/ToDo.md` konsolidasyonu [9. tur], ana sayfa scroll bug'ı + expert çift-modal bug'ı + Zoom 15dk kısıtı + takvim renklendirmesi [8. tur], mobil header/sidebar bug'ları + marka/UI temizliği + client ana sayfası + Clerk sökümü [7. tur], toast z-index bug'ı [6. tur], CSRF koruması [5. tur], access token refresh [3. tur], profil "beyaz sayfa" zinciri [devam turu], randevu 3-ID karışıklığı + login/me `id`/`role` eksikliği [Randevu Zinciri turu], `AvailabilityExceptionView`/`ExpertAvailability` navigate no-op [4. tur]). Tam ayrıntı (kod örnekleri, curl doğrulama adımları) kayıp değil — `git log -p -- CLAUDE.md` ile bu dosyanın o zamanki hâli her zaman geri getirilebilir (repo artık `git subtree` ile birleşmiş tek bir depo, bkz. aşağıdaki "⚠️ Repo Yapısı" notu).
>
...
Notification (notifications/models.py) — 14. tur, YENİ; 16. turda genişletildi
├── user (FK), notification_type (appointment_reminder | message), dedupe_key, title, body
...
3. **[16. turda kullanıcı tarafından açıkça planlandı, henüz uygulanmadı] Seans başına not/mesaj kotası.** 16. turda eklenen `messaging/` app'i şu an sadece genel bir gönderim sıklığı limiti uyguluyor (`30/hour`, tüm konuşmalar için ortak, seans-bağımsız). Kullanıcı, ileride her seans öncesi/sonrası için ayrı bir kota uygulanmasını istiyor — amaç görüntülü görüşmeyi teşvik etmek (not sistemi görüşmenin yerine geçmesin). Bunun için muhtemelen `Message`'a (bilinçli olarak bu turda eklenmeyen) bir `related_appointment` bağlantısı ve randevu durumuna göre bir kota hesaplama mantığı gerekecek — YAGNI gereği bu turda kurulmadı, sadece not düşülüyor.
...
**Son Güncelleme**: 2026-08-22, 19. tur (🔴 kritik: danışan belge/profil fotoğrafı yükleme akışı var olmayan bir endpoint'e istek attığı için tamamen bozuktu — expert'in doğru akışıyla aynı 3 adımlı [presign→PUT→finalize] yapıya çevrildi, gerçek Docker+Supabase'e karşı uçtan uca doğrulandı; bu süreçte `.env`'deki bozuk/maskelenmiş Supabase değerleri ve yanlış anahtar tipi [publishable yerine secret gerekiyordu] teşhis edilip düzeltildi; Render+Supabase Frankfurt'ta ortaklaştırılıp gözle görülür bir hızlanma sağlandı; 🟠 `default-avatar.png` eksikliğinin yol açtığı sonsuz istek döngüsü düzeltildi; expert'e eksik Netlify `_redirects` eklendi; kullanıcının 7 maddelik UX listesi [spinner'lar, dropdown sadeleştirme, select kontrast bug'ı, accordion→otomatik-açık vb.] uygulanıp test edilirken bulunan 3 ek bug [uzmanlar arası slot seçim çakışması, randevu gönderiminde çift-tıklama yarışı, bozuk spinner path'i] düzeltildi. Belge yükleme akışı gerçek Supabase'e karşı uçtan uca doğrulandı, geri kalan UX değişiklikleri sadece `tsc`/`vite build` ile; gerçek tarayıcı testi bekliyor. Detay: client/claude.md 17. tur, expert/claude.md 16. tur)
