...
**Son Güncelleme**: 2026-08-20, 17. tur (Notlar sistemindeki saatlik gönderim sınırı [30/hour, 16. tur] kullanıcı isteğiyle seans-bazlı bir mesaj kotasına çevrildi — iki seans arası toplam 5 danışan mesajı hakkı, her tamamlanan seans sonrası yenilenen, `Appointment`/`Message` verisinden hesaplanan (ayrı bir model/alan yok). Danışan tarafında dinamik "Kalan Hak: X/5" göstergesi + açıklama modalı + hak bitince kırmızı border/disable + 200 karakter limiti; uzman tarafında danışanın hakkının parantez içinde gösterilmesi (uzmanın kendi sınırı yok); iki frontend'de de gönderilmemiş taslakların localStorage'da korunması. GET/POST yanıt şekli `client_quota` alanı taşıyacak şekilde değişti (breaking, iki frontend güncellendi). 10 senaryo `APIRequestFactory` ile gerçek DB verisiyle doğrulandı, `manage.py check` temiz, migration gerekmedi, iki frontend'de `tsc`/`vite build` temiz; gerçek tarayıcı testi bekliyor)
